...
<template>
	<div class="main-content" :style='{"padding":"50px 30px 30px"}'>
		...
		<template v-if="showFlag ">
			<el-form class="center-form-pv" :style='{"border":"1px solid #e7e9ec","padding":"20px 20px 0","boxShadow":"0 3px 6px rgba(0,0,0,.06)","margin":"0 0 20px","borderRadius":"10px","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","justifyContent":"space-between"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"width":"auto","margin":"0px 0 20px -4px","flexWrap":"wrap","display":"flex"}'>
					...
					<el-button class="add" v-if="isAuth('jinqueziyuan','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia16" :style='{"margin":"0 0px","fontSize":"16px","color":"#5d4037","height":"auto"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('jinqueziyuan','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"border":"1px solid #e7e9ec","width":"100%","padding":"20px","boxShadow":"0 3px 6px rgba(0,0,0,.06)","borderRadius":"10px","background":"#fff"}'>
				<el-table class="tables"
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('jinqueziyuan','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 0px","fontSize":"16px","color":"#333","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="btn8" v-if="isAuth('jinqueziyuan','排队申请')" @click="paiduishenqingCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 0px","fontSize":"14px","color":"#5d4037","height":"40px"}'></span>
								排队申请
							</el-button>
							<el-button class="edit" v-if=" isAuth('jinqueziyuan','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
								<span class="icon iconfont icon-xiugai1" :style='{"margin":"0 0px","fontSize":"16px","color":"#8ebc74","height":"40px"}'></span>
								修改
							</el-button>

							<el-button class="view" v-if="isAuth('jinqueziyuan','查看评论')" type="success" @click="disscussListHandler(scope.row.id)">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 0px","fontSize":"16px","color":"#333","height":"40px"}'></span>
								查看评论
							</el-button>
							...
							<el-button class="del" v-if="isAuth('jinqueziyuan','删除')" type="primary" @click="deleteHandler(scope.row.id)">
		...
		<paiduishenqing-cross-add-or-update v-if="paiduishenqingCrossAddOrUpdateFlag" :parent="this" ref="paiduishenqingCrossaddOrUpdate"></paiduishenqing-cross-add-or-update>
...
<script>
	...
	import paiduishenqingCrossAddOrUpdate from "../paiduishenqing/add-or-update";
	import {
		Loading
	} from 'element-ui';
	export default {
		data() {
			return {
				...
				paiduishenqingCrossAddOrUpdateFlag: false,
		...
		components: {
			AddOrUpdate,
			paiduishenqingCrossAddOrUpdate,
		},
		methods: {
			queryChange(arr){
				for(let x in arr) {
			...
			paiduishenqingCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.paiduishenqingCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','jinqueziyuan');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.paiduishenqingCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.paiduishenqingCrossaddOrUpdate.init(row.id,type);
				});
			},
			init () {
			},
			...
			disscussListHandler(id,type) {
				this.$router.push({path:'/discussjinqueziyuan',query:{refid:id}});
			},
			// 删除
