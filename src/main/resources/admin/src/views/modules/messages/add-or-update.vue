		...
		>
			<template >
				...
				<el-form-item class="upload" v-else-if="ruleForm.rpicture" label="回复图片" prop="rpicture" >
					<img v-if="ruleForm.rpicture.substring(0,4)=='http'&&ruleForm.rpicture.split(',w').length>1" class="upload-img" style="margin-right:20px;" v-bind:key="index" :src="ruleForm.rpicture" width="100" height="100" @click="imgPreView(ruleForm.tupian)">
					<img v-else-if="ruleForm.rpicture.substring(0,4)=='http'" class="upload-img" style="margin-right:20px;" v-bind:key="index" :src="ruleForm.rpicture.split(',')[0]" width="100" height="100" @click="imgPreView(ruleForm.tupian.split(',')[0])">
					<img v-else class="upload-img" style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.rpicture.split(',')" :src="$base.url+item" width="100" height="100" @click="imgPreView($base.url+item)">
				</el-form-item>
			</template>
			<el-form-item v-if="type=='msg'" label="留言内容" prop="content" class="textBox">
				<span class="text ql-snow ql-editor" v-html="ruleForm.content"></span>
			</el-form-item>
			<el-form-item v-else label="留言内容" prop="content" class="editorBox">
				<editor 
