		...
		>
			<template >
				<el-form-item class="input" v-if="type!='info'"  label="申请编号" prop="shenqingbianhao" >
					<el-input v-model="ruleForm.shenqingbianhao" placeholder="申请编号" clearable  :readonly="ro.shenqingbianhao"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="申请编号" prop="shenqingbianhao" >
				...
				<el-form-item class="input" v-if="type!='info'"  label="排队序号" prop="paiduixuhao" >
					<el-input v-model.number="ruleForm.paiduixuhao" placeholder="排队序号" clearable  :readonly="ro.paiduixuhao"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="排队序号" prop="paiduixuhao" style="display:none">
					<el-input v-model="ruleForm.paiduixuhao" placeholder="排队序号" readonly></el-input>
				</el-form-item>
				<el-form-item class="input" v-if="type!='info'" label="目前排序" prop="muqianpaixu" >
					<el-input v-model="muqianpaixu" placeholder="目前排序" readonly></el-input>
				</el-form-item>
				<el-form-item class="input" v-else-if="ruleForm.muqianpaixu" label="目前排序" prop="muqianpaixu" >
					<el-input v-model="ruleForm.muqianpaixu" placeholder="目前排序" readonly></el-input>
				</el-form-item>
	...
	export default {
		...
		computed: {
			...
			muqianpaixu : {
				get: function () {
					let c = this.ruleForm
					let a = c.paiduixuhao-1
					this.ruleForm.muqianpaixu = a?Number(Number(a).toFixed(2)):0
		...
		methods: {
			...
			init(id,type ) {
				...
				if(this.type=='info'||this.type=='else'||this.type=='msg'){
					...
					for (var o in obj){
						...
						if(o=='muqianpaixu'){
							this.ruleForm.muqianpaixu = obj[o];
							this.ro.muqianpaixu = true;
							continue;
						}
						if(o=='yonghuzhanghao'){
			...
			async onSubmit() {
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							if(this.ruleForm.muqianpaixu==0){
								this.$message.error('目前排序不能为空')
								return false
							}
							...
							if(this.type=='cross') {
								await this.$http.post('updateColumn/paiduishenqing/2',{
									csuUpdateColumn: 'paiduixuhao',
									csuUpdateColumnValue: '-1',
							...
							await this.$http({
								url: `paiduijindu/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.paiduijinduCrossAddOrUpdateFlag = false;
											this.parent.search();
										}
									});
								} else {
