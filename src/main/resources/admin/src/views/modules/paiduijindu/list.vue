		...
		<template v-if="showFlag ">
			<el-form class="center-form-pv" :style='{"border":"1px solid #e7e9ec","padding":"20px 20px 0","boxShadow":"0 3px 6px rgba(0,0,0,.06)","margin":"0 0 20px","borderRadius":"10px","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","justifyContent":"space-between"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"width":"auto","margin":"0px 0 20px -4px","flexWrap":"wrap","display":"flex"}'>
					...
					<el-button class="add" v-if="isAuth('paiduijindu','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia16" :style='{"margin":"0 0px","fontSize":"16px","color":"#5d4037","height":"auto"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('paiduijindu','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"border":"1px solid #e7e9ec","width":"100%","padding":"20px","boxShadow":"0 3px 6px rgba(0,0,0,.06)","borderRadius":"10px","background":"#fff"}'>
				<el-table class="tables"
					...
					<el-table-column :resizable='true' :sortable='true'
						class-name="showColor1"						prop="muqianpaixu"
						label="目前排序">
						<template slot-scope="scope">
							{{scope.row.muqianpaixu}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'
												prop="gengxinshijian"
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('paiduijindu','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 0px","fontSize":"16px","color":"#333","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="edit" v-if=" isAuth('paiduijindu','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('paiduijindu','删除')" type="primary" @click="deleteHandler(scope.row.id)">
		...
		methods: {
			...
			tableRowClassName({
			  ...
			  rowIndex
				...
				if((Number('0')<Number('1')&&row.muqianpaixu>=Number('0')&&row.muqianpaixu<=Number('1'))||(Number('0')>Number('1')&&(row.muqianpaixu>=Number('0')||row.muqianpaixu<=Number('1')))) {
					name = name + ' ' + 'muqianpaixu-row';
				}
				return name;
			},
			changeClick(e) {
			...
			init () {
				let urlmuqianpaixu = 'paiduijindu/remind/muqianpaixu/1';
				urlmuqianpaixu+=`?remindend=1`,

				urlmuqianpaixu+=`&remindstart=0`,
				...
				this.$http({
					...
					if (data && data.code == 0 && data.count>0 ) {
						...
						var name = data.data
						this.muqianpaixuRemind = data.data
						var arr = []
						...
						this.$msgbox({
							message: h('p', null, [
								h('div', { style: 'text-align: center;font-weight: bold;font-size: 20px;padding: 20px 0' }, `${data.count}条目前排序预警`),
								h('p', null, [
									h('div', { style: 'text-align: center;font-weight: bold;padding: 0 0 10px;font-size: 16px' }, `排队到号，请及时领取！`),
...
<style lang="scss" scoped>
	...
	::v-deep .el-table .muqianpaixu-row .showColor1 {
