	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="input" v-if="type!='info'"  label="排队人数" prop="paiduirenshu" >
					<el-input v-model.number="ruleForm.paiduirenshu" placeholder="排队人数" clearable  :readonly="ro.paiduirenshu"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="排队人数" prop="paiduirenshu" >
					<el-input v-model="ruleForm.paiduirenshu" placeholder="排队人数" readonly></el-input>
				</el-form-item>
				<el-form-item class="input" v-if="type!='info'" label="排队序号" prop="paiduixuhao" >
					<el-input v-model="paiduixuhao" placeholder="排队序号" readonly></el-input>
				</el-form-item>
				<el-form-item class="input" v-else-if="ruleForm.paiduixuhao" label="排队序号" prop="paiduixuhao" >
			...
			async onSubmit() {
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `paiduishenqing/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.paiduishenqingCrossAddOrUpdateFlag = false;
											this.parent.search();
										}
									});
								} else {
