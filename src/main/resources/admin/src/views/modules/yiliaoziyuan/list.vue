...
<template>
	<div class="main-content" :style='{"padding":"50px 30px 30px"}'>
		...
		<template v-if="showFlag ">
			<el-form class="center-form-pv" :style='{"border":"1px solid #e7e9ec","padding":"20px 20px 0","boxShadow":"0 3px 6px rgba(0,0,0,.06)","margin":"0 0 20px","borderRadius":"10px","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","justifyContent":"space-between"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"0px","boxShadow":"none","borderRadius":"10px","flexWrap":"wrap","display":"flex","width":"auto","position":"relative"}' >
					...
					<div  :style='{"margin":"0 10px 20px 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"16px","fontWeight":"500","height":"40px"}' class="item-label">资源用途</label>
						<el-input v-model="searchForm.ziyuanyongtu" placeholder="资源用途" @keydown.enter.native="search()" clearable></el-input>
					</div>
				...
				<el-row class="actions" :style='{"width":"auto","margin":"0px 0 20px -4px","flexWrap":"wrap","display":"flex"}'>
					...
					<el-button class="add" v-if="isAuth('yiliaoziyuan','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia16" :style='{"margin":"0 0px","fontSize":"16px","color":"#5d4037","height":"auto"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('yiliaoziyuan','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"border":"1px solid #e7e9ec","width":"100%","padding":"20px","boxShadow":"0 3px 6px rgba(0,0,0,.06)","borderRadius":"10px","background":"#fff"}'>
				<el-table class="tables"
					...
					<el-table-column :resizable='true' :sortable='true'
						class-name="showColor1"						prop="ziyuanshuliang"
						label="资源数量">
						<template slot-scope="scope">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('yiliaoziyuan','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 0px","fontSize":"16px","color":"#333","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="btn8" v-if="isAuth('yiliaoziyuan','立即申请')" @click="ziyuanshenqingCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 0px","fontSize":"14px","color":"#5d4037","height":"40px"}'></span>
								立即申请
							</el-button>
							<el-button class="edit" v-if=" isAuth('yiliaoziyuan','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
								<span class="icon iconfont icon-xiugai1" :style='{"margin":"0 0px","fontSize":"16px","color":"#8ebc74","height":"40px"}'></span>
								修改
							</el-button>

							<el-button class="view" v-if="isAuth('yiliaoziyuan','查看评论')" type="success" @click="disscussListHandler(scope.row.id)">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 0px","fontSize":"16px","color":"#333","height":"40px"}'></span>
								查看评论
							</el-button>
							...
							<el-button class="del" v-if="isAuth('yiliaoziyuan','删除')" type="primary" @click="deleteHandler(scope.row.id)">
		...
		<ziyuanshenqing-cross-add-or-update v-if="ziyuanshenqingCrossAddOrUpdateFlag" :parent="this" ref="ziyuanshenqingCrossaddOrUpdate"></ziyuanshenqing-cross-add-or-update>
...
<script>
	...
	import ziyuanshenqingCrossAddOrUpdate from "../ziyuanshenqing/add-or-update";
	import {
		Loading
	} from 'element-ui';
	export default {
		data() {
			return {
				...
				ziyuanshenqingCrossAddOrUpdateFlag: false,
				layouts: ["total","prev","pager","next","sizes","jumper"],
				previewImg: '',
				previewVisible: false,
				ziyuanshuliangRemind: [],
			};
		},
		created() {
			if(this.statType) {
		...
		components: {
			AddOrUpdate,
			ziyuanshenqingCrossAddOrUpdate,
		},
		methods: {
			queryChange(arr){
				for(let x in arr) {
			...
			tableRowClassName({
			  ...
			  rowIndex
				...
				let name = ''
				let title = ''
				title = 'ziyuanmingcheng'
				if(row.ziyuanshuliang<=Number('3')) {
					name = name + ' ' + 'ziyuanshuliang-row';
				}
				return name;
			},
			changeClick(e) {
				// 提醒标题有吗
				this.searchForm.ziyuanmingcheng = e
				this.search()
				// 关闭弹窗
				this.$msgbox.close()
			},
			...
			ziyuanshenqingCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.ziyuanshenqingCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','yiliaoziyuan');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.ziyuanshenqingCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.ziyuanshenqingCrossaddOrUpdate.init(row.id,type);
				});
			},
			init () {
			...
			getDataList() {
				...
				if(this.searchForm.ziyuanyongtu!='' && this.searchForm.ziyuanyongtu!=undefined){
					params['ziyuanyongtu'] = '%' + this.searchForm.ziyuanyongtu + '%'
				}
				let user = JSON.parse(this.$storage.getObj('userForm'))
				this.$http({
					url: "yiliaoziyuan/page",
					method: "get",
					params: params
				}).then(({ data }) => {
					if (data && data.code === 0) {
			...
			disscussListHandler(id,type) {
				this.$router.push({path:'/discussyiliaoziyuan',query:{refid:id}});
			},
			// 删除
...
<style lang="scss" scoped>
	...
	::v-deep .el-table .ziyuanshuliang-row .showColor1 {
