...
<template>
	<div class="addEdit-block">
		...
		>
			<template >
				<el-form-item class="input" v-if="type!='info'"  label="订单编号" prop="shenqingbianhao" >
					<el-input v-model="ruleForm.shenqingbianhao" placeholder="订单编号" clearable  :readonly="ro.shenqingbianhao"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="订单编号" prop="shenqingbianhao" >
					<el-input v-model="ruleForm.shenqingbianhao" placeholder="订单编号" readonly></el-input>
				</el-form-item>
				<el-form-item class="input" v-if="type!='info'"  label="资源编号" prop="ziyuanbianhao" style="display:none">
					<el-input v-model="ruleForm.ziyuanbianhao" placeholder="资源编号" clearable  :readonly="ro.ziyuanbianhao"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="资源编号" prop="ziyuanbianhao" style="display:none">
				...
				<el-form-item class="input" v-if="type!='info'"  label="资源分类" prop="ziyuanfenlei" >
					<el-input v-model="ruleForm.ziyuanfenlei" placeholder="资源分类" clearable  :readonly="ro.ziyuanfenlei"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="资源分类" prop="ziyuanfenlei" >
					<el-input v-model="ruleForm.ziyuanfenlei" placeholder="资源分类" readonly></el-input>
				</el-form-item>
				...
				<el-form-item class="input" v-if="type!='info'"  label="取消数量" prop="ziyuanshuliang" >
					<el-input v-model.number="ruleForm.ziyuanshuliang" placeholder="取消数量" clearable  :readonly="ro.ziyuanshuliang"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="取消数量" prop="ziyuanshuliang" >
					<el-input v-model="ruleForm.ziyuanshuliang" placeholder="取消数量" readonly></el-input>
				</el-form-item>
				<el-form-item class="input" v-if="type!='info'" label="取消金额" prop="tuidingjine" >
					<el-input v-model="tuidingjine" placeholder="取消金额" readonly></el-input>
				</el-form-item>
				<el-form-item class="input" v-else-if="ruleForm.tuidingjine" label="取消金额" prop="tuidingjine" >
					<el-input v-model="ruleForm.tuidingjine" placeholder="取消金额" readonly></el-input>
				</el-form-item>
				...
				<el-form-item class="input" v-if="type!='info'"  label="医生工号" prop="yishenggonghao" style="display:none">
		...
		computed: {
			...
			tuidingjine : {
				get: function () {
					...
					this.ruleForm.tuidingjine = a?Number(Number(a).toFixed(2)):0
			...
			init(id,type ) {
				...
				if(this.type=='info'||this.type=='else'||this.type=='msg'){
					...
					for (var o in obj){
						...
						if(o=='tuidingjine'){
							this.ruleForm.tuidingjine = obj[o];
							this.ro.tuidingjine = true;
							continue;
						}
						if(o=='quxiaoyuanyin'){
			...
			async onSubmit() {
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							if(this.ruleForm.tuidingjine==0){
								this.$message.error('取消金额不能为空')
								return false
							}
							...
							let errMsg = ''
							if(this.type=='cross') {
								await this.$http.post('updateColumn/ziyuanshenqing/2',{
									csuUpdateColumn: 'ziyuanshuliang',
									csuUpdateColumnValue: '-' + this.ruleForm.ziyuanshuliang,
									csuConditionColumn: 'shenqingbianhao',
									csuConditionColumnValue: this.ruleForm.shenqingbianhao
								}).then(rs=>{
									if(rs.data.code!=0) {
										changeType = false
										errMsg = '订单数量删减失败'
							...
							await this.$http({
								url: `yuyuequxiao/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.yuyuequxiaoCrossAddOrUpdateFlag = false;
											this.parent.search();
										}
									});
								} else {
