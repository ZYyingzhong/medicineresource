		...
		<template v-if="showFlag ">
			<el-form class="center-form-pv" :style='{"border":"1px solid #e7e9ec","padding":"20px 20px 0","boxShadow":"0 3px 6px rgba(0,0,0,.06)","margin":"0 0 20px","borderRadius":"10px","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","justifyContent":"space-between"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"0px","boxShadow":"none","borderRadius":"10px","flexWrap":"wrap","display":"flex","width":"auto","position":"relative"}' >
					...
					<div v-if="queryChange(['管理员'])" :style='{"margin":"0 10px 20px 0","display":"flex"}'>
				...
				<el-row class="actions" :style='{"width":"auto","margin":"0px 0 20px -4px","flexWrap":"wrap","display":"flex"}'>
					...
					<el-button class="add" v-if="isAuth('yuyuequxiao','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia16" :style='{"margin":"0 0px","fontSize":"16px","color":"#5d4037","height":"auto"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('yuyuequxiao','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-shanchu6" :style='{"margin":"0 0px","fontSize":"16px","color":"#db562d","height":"auto"}'></span>
						删除
					</el-button>


					<el-button class="btn18" v-if="isAuth('yuyuequxiao','审核')" :disabled="dataListSelections.length?false:true" type="success" @click="shBatchDialog()">
						<span class="icon iconfont icon-shenhe9" :style='{"margin":"0 0px","fontSize":"16px","color":"#5d4037","height":"auto"}'></span>
						审核
					</el-button>
					<el-button class="btn18" v-if="isAuth('yuyuequxiao','支付')" :disabled="dataListSelections.length?false:true" type="success" @click="payBatch()">
				...
				<el-table class="tables"
					...
					<el-table-column :resizable='true' :sortable='true' prop="ispay" label="是否支付">
						<template slot-scope="scope">
							...
							<el-button v-if="scope.row.ispay!='已支付' && isAuth('yuyuequxiao','支付') " type="text" size="small" @click="payHandler(scope.row)">支付</el-button>
						</template>
					</el-table-column>
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('yuyuequxiao','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 0px","fontSize":"16px","color":"#333","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="edit" v-if=" isAuth('yuyuequxiao','修改')  && scope.row.sfsh=='待审核' " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('yuyuequxiao','删除')" type="primary" @click="deleteHandler(scope.row.id)">
	...
	export default {
		...
		methods: {
			...
			payHandler(row){
				...
				this.$storage.set('paytable','yuyuequxiao');
			...
			payBatch(){
				...
				this.$confirm('是否支付所选订单？').then(async _ => {
					...
					for(let i in arr){
						...
						let changeType = true
						let errMsg = ''
						await this.$http.post('deleteColumn/ziyuanshenqing',{
							csuConditionColumn: 'shenqingbianhao',
							csuConditionColumnValue: arr[i].shenqingbianhao
						}).then(rs=>{})
						if(!changeType) {
				...
				this.$refs["shBatchForm"].validate(valid => {
					if(valid){
						this.$confirm(`是否${this.batchIds.length>1?'一键审核':'审核'}选中数据?`, "提示", {
							...
							this.$http({
								url: "yuyuequxiao/shBatch?sfsh="+this.shBatchForm.sfsh+"&shhf="+this.shBatchForm.shhf,
								method: "post",
								data: this.batchIds
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
