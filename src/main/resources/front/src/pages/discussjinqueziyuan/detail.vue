	...
	<div>
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'→'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/discussjinqueziyuan?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
			...
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						...
						<div class="text">
							<span class="ql-snow ql-editor" v-html="detail.content"></span>
						</div>
					</div>
					<div class="item">
						<div class="lable">回复内容</div>
						<div class="text">
							<span class="ql-snow ql-editor" v-html="detail.reply"></span>
						</div>
					</div>
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('discussjinqueziyuan','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('discussjinqueziyuan','删除')" @click="delClick">删除</el-button>
						<el-button class="editBtn" @click="discussClick()" type="warning" v-if="btnAuth('discussjinqueziyuan','查看评论')">查看评论</el-button>
					</div>
				</div>
			</div>
...
<script>
	...
	export default {
		...
		methods: {
			...
			// 修改
			editClick(){
				this.$router.push(`/index/discussjinqueziyuanAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此紧缺资源评论？') .then(_ => {
					this.$http.post('discussjinqueziyuan/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
