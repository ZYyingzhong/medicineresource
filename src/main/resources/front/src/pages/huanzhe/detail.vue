	...
	<div>
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'→'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/huanzhe?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('huanzhe','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('huanzhe','删除')" @click="delClick">删除</el-button>
						<el-button class="reserveBtn" 
							v-if="btnAuth('huanzhe','诊疗方案')" 
							@click="onAcross('zhenliaofangan','','','','')" type="warning">
							诊疗方案
						</el-button>
						<el-button class="editBtn" @click="allchapterClick()" type="warning" v-if="btnAuth('huanzhe','章节管理')">章节管理</el-button>
					</div>
				</div>
			</div>
			...
			<el-tabs id="tabsBox" class="detail-tabs" v-model="activeName" type="border-card" v-if="tabsNum>0" >
				<el-tab-pane label="病史" name="1">
					<div class="ql-snow ql-editor" v-html="detail.bingshi"></div>
				</el-tab-pane>
			</el-tabs>
		...
		methods: {
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/huanzhe', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 修改
			editClick(){
				this.$router.push(`/index/huanzheAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此患者？') .then(_ => {
					this.$http.post('huanzhe/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
			...
			allchapterClick (){
				...
				this.$router.push({path: '/index/chapterhuanzhe', query: params});
			},
			// 获取uuid
			getUUID() {
