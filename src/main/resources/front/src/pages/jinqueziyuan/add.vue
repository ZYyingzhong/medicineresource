		...
		methods: {
			...
			init(type) {
				...
				if(type=='cross'){
					...
					this.ruleForm.paiduirenshu = Number('0'); 				}else if(type=='edit'){
					this.info()
				}
				// 获取用户信息
			...
			// 多级联动参数
			async info() {
				await this.$http.get(`jinqueziyuan/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit(subMitType=null) {
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`jinqueziyuan/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								await this.$message({
									message: '操作成功',
									type: 'success',
