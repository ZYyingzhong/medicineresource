		...
		<el-form
			...
			<el-form-item class="add-item" label="申请编号" prop="shenqingbianhao">
				<el-input v-model="ruleForm.shenqingbianhao" 
					placeholder="申请编号" clearable :readonly="ro.shenqingbianhao"></el-input>
			</el-form-item>
			...
			<el-form-item class="add-item" label="排队序号" prop="paiduixuhao">
				<el-input v-model.number="ruleForm.paiduixuhao" 
					placeholder="排队序号" clearable :readonly="ro.paiduixuhao"></el-input>
			</el-form-item>
			<el-form-item class="add-item" label="目前排序" prop="muqianpaixu">
				<el-input v-model="muqianpaixu" placeholder="目前排序" disabled></el-input>
			</el-form-item>
			<el-form-item class="add-item" label="更新时间" prop="gengxinshijian">
				<el-date-picker
					:disabled="ro.gengxinshijian"
...
<script>
	export default {
		...
		methods: {
			...
			// 多级联动参数
			async info() {
				await this.$http.get(`paiduijindu/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit(subMitType=null) {
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`paiduijindu/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								await this.$message({
									message: '操作成功',
									type: 'success',
