	...
	<div>
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'→'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/paiduijindu?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">目前排序</div>
						<div class="text "  >{{detail.muqianpaixu}}</div>
					</div>
					<div class="item">
						<div class="lable">更新时间</div>
						<div class="text "  >{{detail.gengxinshijian}}</div>
					</div>
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('paiduijindu','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('paiduijindu','删除')" @click="delClick">删除</el-button>
						<el-button class="editBtn" @click="allchapterClick()" type="warning" v-if="btnAuth('paiduijindu','章节管理')">章节管理</el-button>
					</div>
				</div>
			</div>
		...
		methods: {
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/paiduijindu', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 修改
			editClick(){
				this.$router.push(`/index/paiduijinduAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此排队进度？') .then(_ => {
					this.$http.post('paiduijindu/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
			...
			allchapterClick (){
				...
				this.$router.push({path: '/index/chapterpaiduijindu', query: params});
			},
			// 获取uuid
			getUUID() {
