					...
					<el-table-column :resizable='true' prop="image" width="200" label="头像">
						<template slot-scope="scope">
							<div v-if="scope.row.image">
								...
								<img v-else :src="baseUrl+scope.row.image.split(',')[0]" width="100" height="100" style="object-fit: cover" @click="imgPreView(baseUrl+scope.row.image.split(',')[0])">
							</div>
							<div v-else>无图片</div>
						</template>
					</el-table-column>
	...
	export default {
		...
		methods: {
			...
			getList(page, fenlei, ref = '') {
				...
				this.$http.get(`users/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(async res => {
					if (res.data.code == 0) {
