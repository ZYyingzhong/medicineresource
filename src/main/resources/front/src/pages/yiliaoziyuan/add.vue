...
<template>
	<div class="add-update-preview">
		<el-form
			...
			label-width="200px"
			>
			<el-form-item class="add-item" label="资源编号" prop="ziyuanbianhao">
				<el-input v-model="ruleForm.ziyuanbianhao" placeholder="资源编号" disabled></el-input>
			</el-form-item>
			...
			<el-form-item class="add-item"  label="资源分类" prop="ziyuanfenlei">
				<el-select v-model="ruleForm.ziyuanfenlei" placeholder="请选择资源分类" :disabled="ro.ziyuanfenlei"  filterable>
					<el-option
			...
			<el-form-item class="add-item" label="资源数量" prop="ziyuanshuliang">
				<el-input v-model.number="ruleForm.ziyuanshuliang" 
					placeholder="资源数量" clearable :readonly="ro.ziyuanshuliang"></el-input>
			</el-form-item>
			<el-form-item class="add-item" label="资源用途" prop="ziyuanyongtu">
				<el-input
					type="textarea"
					:rows="8"
					:disabled="ro.ziyuanyongtu"
					placeholder="资源用途"
					v-model="ruleForm.ziyuanyongtu">
					</el-input>
			</el-form-item>
			<el-form-item class="add-item" label="资源简介" prop="ziyuanjianjie">
				<editor 
	...
	export default {
		...
		methods: {
			...
			init(type) {
				...
				this.$http.get('option/ziyuanfenlei/ziyuanfenlei', {}).then(res => {
					if (res.data.code == 0) {
			...
			// 多级联动参数
			async info() {
				await this.$http.get(`yiliaoziyuan/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit(subMitType=null) {
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`yiliaoziyuan/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								await this.$message({
									message: '操作成功',
									type: 'success',
