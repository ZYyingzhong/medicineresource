	...
	<div>
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'→'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/yiliaoziyuan?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('yiliaoziyuan','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('yiliaoziyuan','删除')" @click="delClick">删除</el-button>
						<el-button class="reserveBtn" 
							v-if="btnAuth('yiliaoziyuan','立即申请')" 
							@click="onAcross('ziyuanshenqing','','','','')" type="warning">
							立即申请
						</el-button>
						<el-button class="editBtn" @click="allchapterClick()" type="warning" v-if="btnAuth('yiliaoziyuan','章节管理')">章节管理</el-button>
						<el-button class="editBtn" @click="discussClick()" type="warning" v-if="btnAuth('yiliaoziyuan','查看评论')">查看评论</el-button>
					</div>
				</div>
			</div>
	...
	export default {
		...
		methods: {
			...
			async onAcross(acrossTable,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1){
				localStorage.setItem('crossTable',`yiliaoziyuan`);
			...
			storeup(type) {
				if (type == 1 && !this.isStoreup) {
					...
					this.$http.post('storeup/add', this.storeupParams).then(res => {
						if (res.data.code == 0) {
							...
							this.$http.post('yiliaoziyuan/update', this.detail).then(res => {});
							this.$message({
								type: 'success',
								message: '收藏成功!',
								duration: 1500,
				...
				if (type == -1 && this.isStoreup) {
					this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: '1', refid: this.detail.id, tablename: 'yiliaoziyuan', userid: Number(localStorage.getItem('frontUserid'))}}).then(res => {
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/yiliaoziyuan', query: params});
				}
			},
			// 下载
			download(file ){
			...
			getDiscussList(page,type=1) {
				this.$http.get('discussyiliaoziyuan/list', {
					params: {
						page, 
						limit: this.pageSize, 
			...
			comzanClick(row){
				...
				if(!this.comzanChange(row)){
					...
					this.$http.post('discussyiliaoziyuan/update',row).then(rs=>{
			...
			discussDel(id){
				this.$confirm('是否删除此评论？').then(_ => {
					this.$http.post('discussyiliaoziyuan/delete',[id]).then(res=>{
			...
			addComments(content=null,type=1) {
				...
				this.$http.post('discussyiliaoziyuan/add', data).then(rs2 => {
					if (rs2.data.code == 0) {
						if(type==1) {
							this.form.content = '';
						}
			...
			// 修改
			editClick(){
				this.$router.push(`/index/yiliaoziyuanAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此医疗资源？') .then(_ => {
					this.$http.post('yiliaoziyuan/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
