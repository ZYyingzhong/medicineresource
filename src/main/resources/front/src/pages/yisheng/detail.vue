	...
	<div>
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'→'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/yisheng?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
			...
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						...
						<div class="text "  >{{detail.keshimingcheng}}</div>
					</div>
					<div class="item">
						<div class="lable">职称</div>
						<div class="text "  >{{detail.zhicheng}}</div>
					</div>
					<div class="item">
						<div class="lable">服务时间</div>
						<div class="text "  >{{detail.fuwushijian}}</div>
					</div>
					<div class="item">
						<div class="lable">擅长领域</div>
						<div class="text "  >{{detail.shanzhanglingyu}}</div>
					</div>
					<div class="item">
						<div class="lable">职业履历</div>
						<div class="text "  >{{detail.zhiyelvli}}</div>
					</div>
					<div class="item">
						<div class="lable">执行资格证</div>
						<el-button class="uploadBtn" @click="download(detail.zhixingzigezheng )">点击下载</el-button>
					</div>
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('yisheng','修改')&&detail.sfsh!='是'&&detail.sfsh!='否' " @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('yisheng','删除')" @click="delClick">删除</el-button>
						<el-button class="auditBtn" v-if="btnAuth('yisheng','审核') && (detail.sfsh=='待审核'||!detail.sfsh)" @click="onSH" type="warning">审核</el-button>
						<el-button class="editBtn" @click="allchapterClick()" type="warning" v-if="btnAuth('yisheng','章节管理')">章节管理</el-button>
					</div>
				</div>
			</div>
		...
		methods: {
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/yisheng', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 修改
			editClick(){
				this.$router.push(`/index/yishengAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此医生？') .then(_ => {
					this.$http.post('yisheng/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
			...
			allchapterClick (){
				...
				this.$router.push({path: '/index/chapteryisheng', query: params});
			},
			// 获取uuid
			getUUID() {
