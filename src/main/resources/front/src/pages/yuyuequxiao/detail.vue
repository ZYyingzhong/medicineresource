	...
	<div>
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'→'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/yuyuequxiao?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">取消数量</div>
						<div class="text "  >{{detail.ziyuanshuliang}}</div>
					</div>
					<div class="item">
						<div class="lable">取消金额</div>
						<div class="text "  >{{detail.tuidingjine}}</div>
					</div>
					<div class="item">
						<div class="lable">取消原因</div>
						<div class="text "  >{{detail.quxiaoyuanyin}}</div>
					</div>
					<div class="item">
						<div class="lable">取消时间</div>
						<div class="text "  >{{detail.quxiaoshijian}}</div>
					</div>
					<div class="item">
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('yuyuequxiao','修改')&&detail.sfsh!='是'&&detail.sfsh!='否' " @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('yuyuequxiao','删除')" @click="delClick">删除</el-button>
						<el-button class="auditBtn" v-if="btnAuth('yuyuequxiao','审核') && (detail.sfsh=='待审核'||!detail.sfsh)" @click="onSH" type="warning">审核</el-button>
						<el-button class="payBtn" v-if="(detail.ispay=='未支付'||!detail.ispay)&&btnAuth('yuyuequxiao','支付')" type="warning" size="small" @click="payClick">支付</el-button>
						<el-button class="editBtn" @click="allchapterClick()" type="warning" v-if="btnAuth('yuyuequxiao','章节管理')">章节管理</el-button>
					</div>
				</div>
			</div>
	...
	export default {
		...
		methods: {
			...
			payClick(){
				...
				localStorage.setItem('paytable','yuyuequxiao')
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/yuyuequxiao', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 修改
			editClick(){
				this.$router.push(`/index/yuyuequxiaoAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此预约取消？') .then(_ => {
					this.$http.post('yuyuequxiao/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
			...
			allchapterClick (){
				...
				this.$router.push({path: '/index/chapteryuyuequxiao', query: params});
			},
			// 获取uuid
			getUUID() {
