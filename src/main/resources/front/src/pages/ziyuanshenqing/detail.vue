	...
	<div>
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'→'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/ziyuanshenqing?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">申请编号</div>
						<div class="text "  >{{detail.shenqingbianhao}}</div>
					</div>
					<div class="item">
					...
					<div class="item">
						<div class="lable">申请数量</div>
						<div class="text "  >{{detail.ziyuanshuliang}}</div>
					</div>
					<div class="item">
						<div class="lable">申请费用</div>
						<div class="text "  >{{detail.shenqingfeiyong}}</div>
					</div>
					<div class="item">
						<div class="lable">申请原因</div>
						<div class="text "  >{{detail.shenqingyuanyin}}</div>
					</div>
					<div class="item">
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('ziyuanshenqing','修改')&&detail.sfsh!='是'&&detail.sfsh!='否' " @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('ziyuanshenqing','删除')" @click="delClick">删除</el-button>
						<el-button class="reserveBtn" 
							v-if="btnAuth('ziyuanshenqing','取消预约')" 
							@click="onAcross('yuyuequxiao','是','是','','')" type="warning">
							取消预约
						</el-button>
						<el-button class="auditBtn" v-if="btnAuth('ziyuanshenqing','审核') && (detail.sfsh=='待审核'||!detail.sfsh)" @click="onSH" type="warning">审核</el-button>
						<el-button class="payBtn" v-if="(detail.ispay=='未支付'||!detail.ispay)&&btnAuth('ziyuanshenqing','支付')" type="warning" size="small" @click="payClick">支付</el-button>
						<el-button class="editBtn" @click="allchapterClick()" type="warning" v-if="btnAuth('ziyuanshenqing','章节管理')">章节管理</el-button>
					</div>
				</div>
			</div>
	...
	export default {
		...
		methods: {
			...
			async onAcross(acrossTable,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1){
				...
				localStorage.setItem('crossTable',`ziyuanshenqing`);
			...
			shHandler(){
				this.$confirm(`确定操作?`, "提示", {
					...
					let changeType = true
					let errMsg = ''
					if(this.shForm.sfsh=='否') {
						await this.$http.post('updateColumn/yiliaoziyuan/2',{
							csuUpdateColumn: 'ziyuanshuliang',
							csuUpdateColumnValue: '+' + this.detail.ziyuanshuliang,
							csuConditionColumn: 'ziyuanbianhao',
							csuConditionColumnValue: this.detail.ziyuanbianhao
						}).then(rs=>{
							if(rs.data.code!=0) {
								changeType = false
								errMsg = '审核失败'
			...
			payClick(){
				...
				localStorage.setItem('paytable','ziyuanshenqing')
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/ziyuanshenqing', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 修改
			editClick(){
				this.$router.push(`/index/ziyuanshenqingAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此资源申请？') .then(_ => {
					this.$http.post('ziyuanshenqing/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
			...
			allchapterClick (){
				...
				this.$router.push({path: '/index/chapterziyuanshenqing', query: params});
			},
			// 获取uuid
			getUUID() {
