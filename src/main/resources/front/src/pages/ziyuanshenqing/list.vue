		...
		<div class="list-preview">
			<el-form :inline="true" :model="formSearch" class="list-form-pv">
				...
				<el-form-item class="list-item" >
					...
					<el-input v-model="formSearch.shenqingyuanyin" placeholder="申请原因" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
				</el-form-item>
				<el-form-item class="list-item">
					<div class="lable">是否支付：</div>
					<el-select v-model="formSearch.ispay" placeholder="是否支付" :clearable="true">
						<el-option v-for="(item, index) in ispayOptions" :key="index" :label="item" :value="item"></el-option>
					</el-select>
				</el-form-item>
				<div class="list-btn-box">
					...
					<el-button class="list-add-btn" v-if="btnAuth('ziyuanshenqing','新增')" type="primary" @click="add('/index/ziyuanshenqingAdd')">
				...
				<div class="list5">
					<div v-for="(item,index) in dataList" :key="index" class="list-item" @click.stop="toDetail(item)" >
						...
						<div class="infoBox">
							...
							<div class="name">申请数量:{{item.ziyuanshuliang}}</div>
	...
	export default {
		...
		methods: {
			...
			getList(page, fenlei, ref = '') {
				...
				if (this.formSearch.shenqingyuanyin != ''&&this.formSearch.shenqingyuanyin != undefined) searchWhere.shenqingyuanyin = '%' + this.formSearch.shenqingyuanyin + '%';
				if(this.formSearch.ispay != ''){
					params['ispay'] = this.formSearch.ispay;
				}
				if(!this.centerType){
					params['sfsh'] = '是';
				}
				...
				this.$http.get(`ziyuanshenqing/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(async res => {
					if (res.data.code == 0) {
			...
			toDetail(item) {
				...
				this.$router.push({path: '/index/ziyuanshenqingDetail', query: params});
			},
